-- Prevent overlapping schedules for the same person + project.
-- Adjacent (touching) ranges are allowed; true date overlap is not.
-- Does not auto-merge adjacent equal-capacity bookings.

-- Resolve any existing overlaps before enforcing the rule.
DO $$
DECLARE
    r RECORD;
    new_end DATE;
BEGIN
    LOOP
        ...
        IF r.a_start < r.b_start THEN
            new_end := r.b_start - 1;
            IF new_end < r.a_start THEN
                DELETE FROM schedule WHERE id = r.a_id;
            ELSE
                UPDATE schedule SET end_date = new_end, updated_at = NOW() WHERE id = r.a_id;
            END IF;
        ELSIF r.b_start < r.a_start THEN
            new_end := r.a_start - 1;
            IF new_end < r.b_start THEN
                DELETE FROM schedule WHERE id = r.b_id;
            ELSE
                UPDATE schedule SET end_date = new_end, updated_at = NOW() WHERE id = r.b_id;
            END IF;
        ELSE
            -- Same start_date: keep the shorter (or equal) row; shift/delete the other.
            IF r.a_end < r.b_end THEN
                UPDATE schedule
                SET start_date = r.a_end + 1, updated_at = NOW()
                WHERE id = r.b_id;
            ELSIF r.b_end < r.a_end THEN
                UPDATE schedule
                SET start_date = r.b_end + 1, updated_at = NOW()
                WHERE id = r.a_id;
            ELSE
                DELETE FROM schedule WHERE id = r.b_id;
            END IF;
...
AS $$
BEGIN
    IF p_person_id IS NULL OR p_project_id IS NULL OR p_start IS NULL OR p_end IS NULL THEN
        RETURN;
    END IF;
    ...
    IF EXISTS (
        ...
        WHERE s.person_id = p_person_id
          ...
          AND (p_exclude_id IS NULL OR s.id IS DISTINCT FROM p_exclude_id)
          AND daterange(s.start_date, s.end_date, '[]') && daterange(p_start, p_end, '[]')
    ) THEN
        RAISE EXCEPTION
            'schedule overlaps another allocation for the same person and project'
            USING ERRCODE = 'exclusion_violation';
    END IF;
END;
$$;

CREATE OR REPLACE FUNCTION prevent_schedule_person_project_overlap()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER schedule_no_person_project_overlap
    BEFORE INSERT OR UPDATE OF person_id, project_id, start_date, end_date
    ON schedule
    FOR EACH ROW
    EXECUTE FUNCTION prevent_schedule_person_project_overlap();

-- Call assert early in upsert for a clear RPC error before write.
...
BEGIN
    ...
    PERFORM assert_schedule_no_person_project_overlap(
        p_person_id, v_req.project_id, v_req.start_date, v_req.end_date, NULL
    );
