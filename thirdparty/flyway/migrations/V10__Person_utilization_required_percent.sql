-- Scale Skill Matcher availability filters by the request's required billable %.
-- complete: avg_availability >= p_required_percent
-- partial:  (0.75 * p_required_percent) <= avg_availability < p_required_percent
-- everyone: no availability filter

DROP FUNCTION IF EXISTS person_utilization_search(DATE, DATE, TEXT, UUID, UUID, UUID, UUID, TEXT, TEXT);

CREATE OR REPLACE FUNCTION person_utilization_search(
    p_from DATE,
    p_to DATE,
    p_availability TEXT DEFAULT 'everyone',
    p_required_percent SMALLINT DEFAULT 100,
...
AS $$
    WITH params AS (
        SELECT
            GREATEST(0, LEAST(100, COALESCE(p_required_percent, 100)))::numeric AS required_percent
    ),
    candidates AS (
        SELECT
            p.id,
    ...
    WHERE
        CASE lower(COALESCE(p_availability, 'everyone'))
            WHEN 'complete' THEN w.avg_availability >= par.required_percent
            WHEN 'partial' THEN
                w.avg_availability >= (par.required_percent * 0.75)
                AND w.avg_availability < par.required_percent
            ELSE TRUE
        END
